...
MapApp = R6Class("MapAppClass",
    ...
    private = list(
        ...
        extractTracksTable = function(tracks.yaml.file){
          xs <- yaml.load(readLines(tracks.yaml.file))$tracks
          parse.yaml <- function(x){
             name <- x$name
             color <- x$color
             group <- x$group
             data.frame(name=name, color=color, group=group, lat=I(list(x$lat)), lon=I(list(x$lon)))
             }
          do.call(rbind, lapply(xs, parse.yaml))
          }
    ...
    public = list(
       ...
       initialize = function(){
          ...
          private$tbl.tracks  <- private$extractTracksTable(tracks.yaml.file)
          trackCategories <- unique(private$tbl.tracks$group)
          printf("--- tbl.tracks")
          print(private$tbl.tracks)

          markerCategories <- unique(private$tbl$group)
          private$featureGroups <- sort(unique(c(markerCategories, regionCategories, trackCategories)))
